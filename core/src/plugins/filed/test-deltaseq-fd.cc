...
static pFuncs pluginFuncs = {
    ...
    setFileAttributes, NULL, /* no checkFile */
    NULL,                    /* no getAcl */
    NULL,                    /* no setAcl */
    NULL,                    /* no getXattr */
    NULL                     /* no setXattr */
...
class delta_test {
 ...
 public:
  ...
  ~delta_test() { FreeAndNullPoolMemory(fname); }
};
...
static const char* files[] = {"/etc/passwd", "/etc/group", "/etc/hosts",
                              "/etc/services"};
  ...
  switch (io->func) {
    case IO_OPEN:
      ...
      if (io->flags & (O_CREAT | O_WRONLY)) {
        ...
        if (!self->fd) {
          io->io_errno = errno;
          Jmsg(ctx, M_FATAL, "Open failed: ERR=%s\n", strerror(errno));
          return bRC_Error;
        }
    ...
    case IO_READ:
      ...
      if (io->status == 0 && ferror(self->fd)) {
        ...
        Dmsg(ctx, debuglevel, "Pipe read error: ERR=%s\n", strerror(errno));
        return bRC_Error;
      }
    ...
    case IO_WRITE:
      ...
      if (io->status == 0 && ferror(self->fd)) {
        Jmsg(ctx, M_FATAL, "Pipe write error\n");
        Dmsg(ctx, debuglevel, "Pipe read error: ERR=%s\n", strerror(errno));
        return bRC_Error;
      }
      break;
