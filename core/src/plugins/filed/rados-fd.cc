...
#define PLUGIN_USAGE                                                          \
  "rados:conffile=<ceph_conf_file>:namespace=<name_space>:clientid=<client_"  \
  "id>:clustername=<clustername>:username=<username>:snapshotname=<snapshot_" \
  "name>:"
...
static bRC freePlugin(bpContext* ctx)
{
  ...
  if (p_ctx->rados_snapshotname) { free(p_ctx->rados_snapshotname); }

  if (p_ctx->rados_namespace) { free(p_ctx->rados_namespace); }

  if (p_ctx->rados_poolname) { free(p_ctx->rados_poolname); }

  if (p_ctx->rados_clientid) { free(p_ctx->rados_clientid); }

  if (p_ctx->rados_clustername) { free(p_ctx->rados_clustername); }

  if (p_ctx->rados_username) { free(p_ctx->rados_username); }

  if (p_ctx->rados_conffile) { free(p_ctx->rados_conffile); }
...
static bRC get_next_object_to_backup(bpContext* ctx)
{
  ...
  while (1) {
    ...
    status = rados_nobjects_list_next(p_ctx->list_iterator, &p_ctx->object_name,
                                      NULL, NULL);
#else
    status = rados_objects_list_next(p_ctx->list_iterator, &p_ctx->object_name,
                                     NULL);
#endif
    if (status < 0) {
      BErrNo be;
...
#if defined(HAVE_RADOS_NAMESPACES) && defined(HAVE_RADOS_NOBJECTS_LIST)
          Jmsg(ctx, M_ERROR,
               "rados-fd: rados_nobjects_list_next() failed: %s\n",
               be.bstrerror(-status));
#else
      ...
      Mmsg(p_ctx->next_filename, "%s/%s", p_ctx->rados_poolname,
           p_ctx->object_name);
    }
...
static bRC endBackupFile(bpContext* ctx)
{
  ...
  if (!p_ctx || !p_ctx->list_iterator) { return bRC_Error; }
...
static bRC connect_to_rados(bpContext* ctx)
{
  ...
  if (!p_ctx->cluster_initialized) {
    ...
    status = rados_create2(&p_ctx->cluster, p_ctx->rados_clustername,
                           p_ctx->rados_username, rados_flags);
#endif
    if (status < 0) {
      BErrNo be;
...
static bRC setup_backup(bpContext* ctx, void* value)
{
  ...
  status = rados_ioctx_snap_lookup(p_ctx->ioctx, p_ctx->rados_snapshotname,
                                   &p_ctx->snap_id);
  if (status < 0) {
    BErrNo be;
...
static bRC pluginIO(bpContext* ctx, struct io_pkt* io)
{
  ...
  switch (io->func) {
    case IO_OPEN:
      ...
      break;
    case IO_READ:
      io_count = rados_read(p_ctx->ioctx, p_ctx->object_name, io->buf,
                            io->count, p_ctx->offset);
      if (io_count >= 0) {
      ...
      break;
    case IO_WRITE:
      io_count = rados_write(p_ctx->ioctx, p_ctx->object_name, io->buf,
                             io->count, p_ctx->offset);
#if LIBRADOS_VERSION_CODE <= 17408
      if (io_count >= 0) {
        p_ctx->offset += io_count;
...
static bRC createFile(bpContext* ctx, struct restore_pkt* rp)
{
  ...
  switch (rp->type) {
    ...
    case FT_DELETED:
      Jmsg(ctx, M_INFO, 0,
           _("rados-fd: Original file %s have been deleted: type=%d\n"),
           rp->ofname, rp->type);
      rp->create_status = CF_SKIP;
      break;
    default:
      Jmsg(ctx, M_ERROR, 0,
           _("rados-fd: Unknown file type %d; not restored: %s\n"), rp->type,
           rp->ofname);
...
static bRC setXattr(bpContext* ctx, xattr_pkt* xp)
{
  ...
  p_ctx->object_name = ++bp;
  status = rados_setxattr(p_ctx->ioctx, p_ctx->object_name, xp->name, xp->value,
                          xp->value_length);
  if (status < 0) {
    BErrNo be;
